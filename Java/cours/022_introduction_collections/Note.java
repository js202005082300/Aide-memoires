...
Pour suivre ce nouveau chapitre, il est inutile de suivre le cours si vous n'avez pas assimilé des notions comme l'héritage, le polymorphisme, le fonctionnement et l'utilité des interfaces, on en a longuement parlé, Je vais essayer d'être un maximum complet et de vous donner un maximum de détails sur cette partie-là pour que vous puissiez comprendre le principe et à quoi ça sert, dans quels cas vous allez pouvoir les employer.
...
Si vous avez des problèmes avec ces différentes notions, ces différents concepts en programmation et plus exactement le langage Java, ça sert à rien de poursuivre car tout ce que vous avez vu évidemment jusqu'à présent va vous servir pour la suite et va vous aider un petit peu à comprendre tout ça. 
...
Les collections c'est un terme qu'on utilise en Java pour présenter pour parler de tout ce qui est structure de données. 

Vous avez peut-être vu dans d'autres langages de programmation Ou simplement en informatique de manière générale, entendu parler de pile, de liste, de file, on a vu précédemment comme manipuler des tableaux à taille fixe notamment sur les premières séances du cours lorsqu'on a parlé des tableaux. 
...
Parce que en fait des collections permettent de stocker ce qu'on appelle des éléments, des différentes données et on peut ajouter de nouveaux éléments, en supprimer, on peut accéder à ces différents éléments, supprimer même un endroit bien précis de la structure et tout ça est géré de manière dynamique au niveau de la mémoire. C'est tout l'intérêt d'ailleurs de se servir de ses collections. 
...
En Java il y a énormément d'interface qui sont rattachés aux collections et je vais vous en présenter quelques-unes au travers d'un schéma pour faire très simple et faire cette petite introduction Aux collections que nous allons voir en Java. 

On commence par ici avec ce petit schéma Qui vous présente en tout cas rapidement 4 grandes familles de collection. 
...
À partir de ces interfaces on va pouvoir retrouver des classes qui vont les implémenter. 
...
On va retrouver par exemple la méthode add(element) qui permettra d'ajouter un élément. Je ne vais pas mettre les valeurs de retour et cetera, le but ce n'est pas de vous énumérer ce que l'on retrouve dans une documentation Mais simplement que vous puissiez voir quelques méthodes. Je vais d'ailleurs ne pas mettre de paramètres. 
...
Voici parmi les classes pour manipuler directement nos colections, on les retrouve ici. On les retrouve ici, je vous en ai énuméré quelques-unes. 
