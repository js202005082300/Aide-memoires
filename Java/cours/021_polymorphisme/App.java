...
Le même schéma vu précédemment c'est-à-dire les 2 classes abstraites, Vehicle et Building, On va voir des classes concrètes qui héritent pour chacune de ces classes abstraites donc Ship, Glider et Cottage. 
Et on aura une interface donc toujours pareil Flyable et avec 2 classes concrètes qui les implémentent. 
...
Donc il faudra définir précisément concrètement le comportement pour chacune des méthodes de l'interface donc ici on en avait qu'une pour rester sur quelque chose d'assez court donc on l'a bien implémenté là.
...
Certes au niveau de classe abstraite comme je l'ai expliqué, donc là ces classes abstraites permettent de factoriser le code donc d'éviter la répétition de code entre classes concrètes étroitement liées entre elles donc on va forcément gagner quelques lignes de code, c'est évident. 
...
Déjà si on n'avait pas le lien, ce serait assez compliqué car comme je vous l'avais expliqué il faudrait que toutes les classes concrète aient en fait leurs méthodes implémentées. Il faudrait que véhicule aient toutes les méthodes implémentées parce que ce serait une classe concrète si on prenait un autre exemple, il faudrait tout refaire pour Ship, Glider et cetera et cetera. Donc plein de répétition Le code, pas de chose très pratique et comme je l'avais expliqué sur la séance concernant les interfaces, si on peut se on n'utilise pas d'interface, en gros il faut qu'on mette du coup la capacité de vol dans certaines classes mais il faudrait quand même de l'héritage pour que ça puisse profiter à l'autre donc bref soit on aura de la répétition de code soit on avait très mauvaise conception en fait au niveau du code. 
...
Ils veulent ensuite le faire voler. Rappelez-vous j'avais mis une énergie pour les paramètres de Fly, je le enlever pour ne pas perdre de temps sur cette vidéo. 
...
Et si demain encore une fois on veut créer une nouvelle classe, on va devoir rajouter tout un tas ... on va devoir déjà modifier, créer une nouvelle classe. Ça c'est déjà une chose, ce n'est pas un souci de créer une nouvelle classe quand on a une nouvelle entité, ça c'est normal, on le ferait dans tous les cas par contre on va devoir modifier également d'autres classes et dans App.java, On va devoir faire tout un tas d'utilisation, bref on va mélanger plein d'informations et on ne va pas forcément respecter chaque donnée. En gros on va commencer, la chaumière va mêler des informations propres navires et inversement alors qu'ils ne sont pas censés faire ça. Ce n'est pas parce que tous les 2 ont la capacité de voller que du coup le navire doit accepter et doit avoir des informations sur la chaumière et inversement. On peut pas commencer à tout mélanger. 
...
Voilà ça au niveau du code, je veux le voir apparaître et là je ne le vois pas et c'est un peu dommage parce que si on utilise pas, on ne fait pas de polymorphisme alors pourquoi on s'en batte à créer cette classe (Vehicle.java) qui a par, mettre des éléments là quel factorise. Elle ne servirait qu'à ça, qu'à factoriser du code. C'est un petit peu dommage de faire une classe juste pour factoriser du code, nous on cherche à plus d'avantages plus de bénéfices donc on va rajouter quelque chose donc pour moi tout ça ce sont des véhicules :
...
Il y en a peut être en voyant ça qu'ils vont se dire bah je ne comprends pas : "là tu crée des véhicules alors que c'est une classe abstraite (abstract public class Vehicle), Tu nous a expliqué avant qu'une classe abstraite on pouvait pas l'instancier.".
...
En gros ça reviendrait à dire que dans un emplacement de véhicule, tu stock un navire et le véhicule c'est la super classe, c'est la classe parentes d'un navire donc on fait du surclassement ou le terme anglais Upcasting s'il y en a qui s'intéressent un petit peu au terme anglais par rapport à ça. Donc les 3 lignes ci-dessus, c'est de Upcasting. 
...
Donc on va faire tout ça. On va créer un tableau de notre interface. Tout ça, c'est assez nouveau pour vous. 
...
Donc on y va, on veut créer une moto. Pareil, ici c'est moi qui travaille tout seul sur le même code mais dites-vous que dans des cas plus réel, on n'est pas forcément un développeur à travailler sur un projet tout seul. Il y a plein d'exemples de projets ou c'est une entreprise complète où il y a 10 20 50 250 personnes qui travaillent sur le même projet donc vous vous doutez bien que les gens ne travaillent pas sur les mêmes fichiers. Chacun a des fichiers à lui et jamais il va avoir besoin d'aller voir ce que fait l'autre donc ces fichiers. C'est important tout fonctionne bien ensemble à la base même si bon Il y a évidemment des bugs comme partout mais on arrive à faire fonctionner tout ce que d'ensemble. 

Donc on y va, on crée une classe moto, on crée Moto.java (copie de Ship.java, renommé en Moto.java).
...
Il y a eu besoin de créer un nouvel élément, terminé. 
...
J'ai créé ma nouvelle classe, ça il faut bien le faire on ne peut pas l'éviter. On crée une nouvelle entité, il faut bien créer le fichier et faire la classe et après là où j'en ai besoin Donc là c'est dans ma méthode main Mais ça peut être ailleurs Dans un autre code, n'importe où, dans n'importe quel fichier. 
Dès que j'ai envie d'utiliser une moto, voilà comme la moto fait partie des éléments capables de voler, pas de problème ça fonctionne.
...
Ça c'est le polymorphisme, tout simplement. 
...
À bientôt pour une nouvelle vidéo sur la chaîne et n'hésitez pas à réagir dans les commentaires si vous avez une question ou quoi que ce soit et à partager bien évidemment ce contenu pour d'autres développeurs qui pourrait se mettre comme vous au langage Java. 
Ciao 
*/
