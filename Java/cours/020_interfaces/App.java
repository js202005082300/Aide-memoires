...
Donc on a se classe là (Ship et Guilder), 2 classes étroitement liées via héritage sur une classe abstraite et à côté une classe toute seule (Cottage) mais j'aurais pu en créer 2, 3, 4, plein d'autres si j'avais voulu pour compléter un peu plus. 
Donc en termes de code, ça se présente comme ça :
...
De là notre navire, quand on crée un bateau, il a une vitesse par défaut qui est indiquée Et moi je me suis décidée pour l'exemple que la vitesse de déplacement, c'est du kilomètre par heure et comme un bateau se déplace en nœuds, ici on aurait quelque chose qui se déplace aux alentours de 9 nœuds ça correspond à grosso modo pour ceux qui seraient curieux, à du 16 kilomètres heure à peu près.
...
J'espère qu'ici c'est clair jusqu'à présent, s'il y a des choses qui ne sont déjà pas clair à partir de là, vous revenez sur la vidéo d'avant concernant la construction d'une classe, les classes abstraites, les attributs, les méthodes, il faut revoir toutes ces choses-là s'il a tout de suite vous êtes déjà perdu. 

Et maintenant, il faut imaginer que c'est la représentation de mon projet et moi j'ai envie de faire un truc, on est d'accord qu'ici en planeur c'est quelque chose qui peut planer donc en gros c'est quelque chose qui peut voler, qui a possibilité de se déplacer dans les airs. 
...
On pourrait d'ailleurs faire une classe spécialisée pour dire que le Glider, le planeur, il ne fait que planer tel quel, et on crée une version plus spécifique avec un planeur avec un système de propulsion mais on ne va pas non plus trop compliquer, on va dire l'exemple (Schéma). 
...
Le problème, c'est que Cottage, une chaumière par rapport à la représentation :
...
En tout cas par rapport à cette représentation, ce ne sont pas des choses étroitement liées. On est parfaitement d'accord pour se dire qu'une chaumière c'est carrément différent d'un navire dans ce cas là. 
...
En gros je vais déjà faire un petit schéma pour vous montrer en quoi ça consisterait simplement. Je vais fournir ce qu'on appelle un contrat, c'est le terme qu'on emploie généralement quand on parle d'interface. 
...
Moi je vous fournis une API, on va créer puisqu'on a le développeur ici, on va créer une interface en Java puisqu'on parle de Java qui est en fait une API pour créer quelque chose de volant, on ne sait pas quoi. C'est juste une interface comme ça : 
...
C'est bien parce que c'est exactement ce que j'ai. Donc si je voulais schématiser en gros. 

Je dirais qu'ici ma chaumière (Cottage), je veux qu'elle puisse voler donc elle va implémenter mon interface Flyale.
...
On implémente l'interface Flyable, attention ce n'est pas de l'héritage, cette l'implémentation Mais comme ici je ne veux pas forcément faire des flèches différentes comme de toute façon je vous parle en même temps que je fais les flèches, vous comprenez bien ce que je veux dire. 

Et la pareil, mon navire il fait ça. 
...
Et du coup dans ces classes qui vont implémenter les interfaces, on dira et bah voilà un temps on va le comparer de telle manière mais le temps virgule on ne va pas le comparer comme des années, on ne va pas non plus le comparer comme un nombre, un Byte, on va pas le comparer comme un double, on va pas le comparer comme un calendrier, on va pas le comparer comme un caractère et cetera et cetera et cetera. 
...
Pour qu'on reste un petit peu dans les conventions de nommage de Java, nous avons appelé notre interface Flyable pour dire volant, c'est un objectif par rapport à ça.
...
On va créer notre fichier qu'on va appeler Flyable.java
...
En gros tout est public et tout est abstrait par défaut donc vous n'avez pas besoin de mettre le mot-clé abstract pour méthode et vous n'avez pas besoin non plus mais vous pouvez toujours le mettre à titre indicatif, le mot clé publique. 
Moi par exemple je ne mettrais que le mot public, C'est assez rare qu'on met le mot clé abstract dans les interfaces mais si vous voulez le mettre il n'y a pas de problème. Si pour vous c'est plus compréhensible de faire comme ça pour être sûr que vous comprenez bien les choses, mettez le il n'y a aucun problème avec ça. 
...
J'ai donc une méthode Fly et par exemple je vais utiliser une chaîne de caractères parce que je ne vais pas créer une autre classe non plus mais genre quelque chose qui représenterait une énergie pour pouvoir voler et c'est tout, mon interface est terminée.
...
Donc voilà si vous avez une hiérarchie d'interfaces comme on aurait une hiérarchie de classes, vous utilisez comme pour les classes, le mots clés extends. par contre dès qu'une classe doit utiliser une interface donc l'implémenter obligatoirement, il faudra utiliser le mot clé "implements".
...
Ça c'est la syntaxe (implements Flyable) pour dire mon avis va implémenter l'interface Flyable par contre comme ça, envoyer que rien n'est implémenté (Flyable.java) :
...
Donc comme pour une classe abstraite, où on aurait pour le coup une classe concrète où Il faudrait implémenter les méthodes abstraite, ici (Ship.java), implémente la méthode abstraite de l'interface pour dire Ben voilà le navire il va voler comme ça.
...
Eh bien je crée une classe concrète qui hérite de la classe abstraite qui a des liens avec s. Si c'est un véhicule il faut que ça hérites de véhicules. Si c'est une sorte de bâtiments, il faut que ça hérites de building. Et si je veux que ça vole, j'implémente tout simplement l'interface Flyable (Fyable.java). Et c'est tout, terminé! 
J'ai pas eu besoin de toucher quoi que ce soit d'autres par rapport à ça. 

Voilà si vous avez compris ce que j'ai dit jusqu à présent parce que je sais que la vidéo pour le cours va être un peu longue parce qu'il y a beaucoup de choses à expliquer. Vous avez déjà compris 99 pour 100 2 interfaces en programmation de manière générale, y compris sur d'autres langages. Parce que le principe encore une fois ce sont des notions élémentaire qui fonctionne de la même manière sur d'autres langages. Après ce qui va changer selon le langage, c'est la syntaxe où comment fonctionne des fois les accès ou autre dans le langage à l'autre. 

Donc ça fait attention Selon le langage de programmation, il y a des choses qui diffèrent mais la notion elle est là : à quoi sert une interface ? C'est ce que je viens de vous dire, c'est exactement pareil pour le c plus plus ou d'autres langages qui par avoir des interfaces. Tout est a fait pareil, il n'y a vraiment pas de différence de ce côté là. 
...
Interface qui permettra à la place du T, de comparer des dates, des maisons, des navires voilà on peut comparer plein de choses Parce que évidemment en termes de code pur "comparer des bateaux", ce n'est pas si simple que ça, ça c'est vraiment humain, il faut réfléchir humainement parlant à comment est-ce qu'on compare de navires? 
...
Et voilà, on peut bien évidemment mettre des éléments, des constantes, rappelez-vous le mot-clé final, on l'a déjà emporté dans la séance précédente.
...
On peut faire des méthodes statiques qui permettent comme ça de passer par l'interface comme vous l'avez vu avec la classe, les classes qui ont des méthodes statiques, sans créer d'instance de quoi que ce soit. 
Donc Flyable, on utilisera la méthode statique, je ne reviens pas dessus on en a déjà parlé tout ce qui était méthode statique. C'est la même chose pour les interfaces. Tout ce que vous savez sur la méthode statique fonctionne pareil pour les interfaces mais c'était quand même bien de le rappeler et pourquoi c'est important ? parce que avant Java 8, Java 7 et les versions antérieures, on ne pouvait pas faire une méthode statique dans une interface c'est pour ça que j'en ai parlé mais de toute façon vous êtes au minimum sur Java 8, voir même Java 10 parce que je ne vous avais dit de ne pas partir sur une version qui date de 5 ou 10 ans. Faut pas non plus déconner donc vous avez quand même une version un minimum récente et c'est pour ça qu'il y a ces petites choses à prendre en compte. 
...
Normalement j'ai fait le tour, il n'y a qu'une dernière chose dont je dois encore vous parler pour la fin de la vidéo mais je regarde si je n'ai pas oublié autre chose de ce que je voulais vous dire, les éléments, on a parlé de ça et cetera. On a fait le tour de tout donc c'est bien. 
...
Il n'y a pas le mot-clé abstract, je rappel. j'aurais même pu mettre comme ça :
...
Donc ici en gros on a ça, on utilise ce mot clé default qui veut dire ben j'implémente mais exceptionnellement, c'est une exception, j'implémente du code dans une interface, chose qui avant Java encore une fois  si je ne me trompe pas, ne permettait pas de le faire. 
...
On compile tout ça, on voit qu'on n'a pas fait d'erreur, on voit que la chaumière qui a implémenté la méthode test(), utilise sa propre méthode et là pour le coup du navire comme il n'a pas implémenté cette méthode et comme elle est marquée comme étant par défaut (Flyable.java) grâce au mot-clé default. Si il n'y est pas vous aurez une erreur évidemment parce qu'il faut bien tout implémenter dans une interface si ce n'est pas une méthode par défaut mais là comme s'en est une Eh bien ce sera cette méthode qui sera utilisée du coup pour le navire.
...
Donc ici "public void fly(String energy)", On comprend que ce n'est pas implémenté mais là " default public void test()", on comprend que ça l'est parce qu'il y a ce petit mot clé magique, le default, qui nous indique que c'est une méthode par défaut.
