...
Mais quand vous avez ce genre de chose vous devez tout écrire de préférence en minuscule, pas de caractères spéciaux, pas d'espace, pas de caractères accentués, et cetera. 
...
par exemple comme package est un mot réservé, vous mettez un petit MY devant et comme ça vous n'aurez pas de problème. 
...
À partir du moment où on a une classe qui fait partie d'un package, il s'agit tout simplement ici de ne pas faire un import, de ne pas mettre un import évidemment. 
...
En espérant que item.java ne soit pas en nous réserver. 
...
À partir de là, nous allons directement compiler ce code. 
...
On exécute notre fichier et on remarque que nos 2 constructeurs ont bien été instancié. On a les 2 constructeurs qui ont été appelés, on a bien réussi à trouver nos 2 classes quoi. 

Donc voilà comment fonctionne la création d'un package. 

C'est très simple et l'utilisation est très simple aussi.

Portée des classes 
==================
On va revenir sur la portée des classes, Il y a des choses qu'on n'avait pas forcément précisé où notamment on avait dit qu'on reviendrait sur certaines choses. Quand on utilisait en fait le mot clé public, On se disait que c'était accessible de partout Et au niveau du package il y a un type de portée à prendre en compte.
En gros si je mets ce mot clé public pour la class ou autre pour d'autres éléments, Ça signifie public pour tout. Ça veut dire pour tout ce qui fait partie à la fois du package Mais aussi tout ce qui est à l'extérieur. 
...
Si des fois vous avez des petites erreurs Où vous avez des accès qui ne fonctionnent pas, ce sera très certainement un problème comme ça où l'on oubliera De spécifier la portée de notre classe Ou d'une méthode Ou d'un attribut d'une classe. Donc très important. 
...
À nous de bien gérer cela et de vérifier encore une fois l'encapsulation des données Comme on l'avait déjà vu précédemment.
...
Du coup on voit ici qu'il y a plusieurs noms séparés par des points et en fait en termes déjà hiérarchie, c'est simplement des sous package et qui sont représentées au niveau de notre explorateur par des sous dossiers. 
...
et au niveau hiérarchie, le changement se fera de cette manière. Donc au lieu d'avoir : 
