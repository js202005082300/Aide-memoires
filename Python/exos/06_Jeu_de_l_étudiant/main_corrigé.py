...
class Player:
	def __init__(self, name, level = 1, golds = 5, HP = 100, MP = 100, EXP = 0, weapon = Weapon("Epee en bois", 32, 1)):
		...
		self.weapon = weapon # /!\ Pour attribut, on a un autre objet avec weapon qui est un attribut de classe Weapon et du coup on peut accéder aux attributs de cet objet.
	...
	name = property(fget = get_name, fset = set_name) #### 12.10
	...
	def level_up(self):
		if self.EXP >= self.next_level_EXP:
			self.level += 1
			self.next_level_EXP += 100 # A chaque monté de niveau, le prochain coûte 100 EXP de plus...
...
while ask_player_name:
	try:
		player_name = input("Choisir un nom (16 caracteres max.) : ") # Unnombientroplongpouretrevalide (trop long)
